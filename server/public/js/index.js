	    ...
	    try {
	        for (var _iterator = Reflect.ownKeys(source)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	            ...
	            if (key !== 'constructor' && key !== 'prototype' && key !== 'name') {
	                var desc = Object.getOwnPropertyDescriptor(source, key); // 返回source对象上自有属性(key)对应的属性描述
	                Object.defineProperty(target, key, desc); // 直接在target对象上定义一个新属性，进而达到深拷贝
	            }
	        }
	    } catch (err) {
	...
	var Base = function () {
	    ...
	    _createClass(Base, [{
	        ...
	        value: function changePlayNav(e) {
	            ...
	            (0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); // 清空操作区已选号码样式
	            self.getCount();
	        }
	        ...
	        value: function assistHandle(e) {
	            ...
	            e.preventDefault();
	            (0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); // 清除
	        ...
	        value: function addCode() {
	            var self = this;
	            var $active = (0, _jquery2.default)('.boll-list .btn-boll-active').text().match(/\d{2}/g); // 数组对象
	        ...
	        value: function getCount() {
	            ...
	            var tpl = void 0;
	            var c1 = win1 < 0 && win2 < 0 ? Math.abs(win1) : win1; // abs()方法返回数的绝对值
	        ...
	        value: function getRandom(num) {
	            var arr = [],
	                index = void 0;
	            var number = Array.from(this.number); // 将set集合转化为数组
	            while (num--) {
	    ...
	    _createClass(Calculate, [{
	        ...
	        value: function computeBonus(active, play_name) {
	            ...
	            var min = void 0,
	                max = void 0;
	            if (play[0] === 'r') {
	                var min_active = 5 - (11 - active); // 最少命中数
	                if (min_active > 0) {
